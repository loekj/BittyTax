...
### Added
- Data conversion tool: added a `--country` option (`UK` default, or `PT`) which controls how forced token conversions (delisting, migration or redenomination) are classified. In `PT` mode they are emitted as a single crypto-to-crypto Trade (a non-taxable swap with cost-basis carryover under Portuguese CIRS rules) rather than separate Airdrop and Spend legs; the Kraken, Binance, Coinbase and Gate.io parsers are supported, low-confidence pairings are flagged for review, and a summary of any reclassifications is logged.
